...
/// <summary>
/// Incremental source generator that scans for classes decorated with [MappingAttribute]
/// and generates extension methods to register their mapping methods with IMap.
/// </summary>
[Generator]
public class RegistrationGenerator : IIncrementalGenerator
{
	...
	private static void Execute(Compilation compilation, ImmutableArray<INamedTypeSymbol> classes, SourceProductionContext context)
	{
		...
		var distinctClasses = classes.Distinct(NamedTypeSymbolComparer.Instance).ToList();
	...
	private static void GenerateRegistrationForClass(INamedTypeSymbol classSymbol, StringBuilder sb)
	{
		...
		var displayClassName = classSymbol.ToDisplayString();
		var instanceName = $"instance_{displayClassName.Replace(".", "_").Replace("<", "_").Replace(">", "_")}";
		...
		foreach (var method in methods)
		{
			...
			{
				var methodAccess = method.IsStatic ? $"{fullyQualifiedClassName}.{method.Name}" : $"{instanceName}.{method.Name}";
		...
		if (registrationCalls.Count > 0)
		{
			...
			{
				sb.AppendLine($"            var {instanceName} = map.Create<{fullyQualifiedClassName}>();");
			}
